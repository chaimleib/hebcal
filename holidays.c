...
holinput_t inp_holidays[] = {
    {{TISHREI, 2, IGNORE_YEAR}, "Rosh Hashana II", YOM_TOV_ENDS, NULL},
    {{TISHREI, 9, IGNORE_YEAR}, "Erev Yom Kippur", LIGHT_CANDLES, NULL},
    {{TISHREI, 10, IGNORE_YEAR}, "Yom Kippur", YOM_TOV_ENDS, NULL},

    {{TISHREI, 14, IGNORE_YEAR}, "Erev Sukkot", LIGHT_CANDLES, NULL},
    {{TISHREI, 15, IGNORE_YEAR}, "Sukkot I", CHUL_ONLY | LIGHT_CANDLES_TZEIS, NULL},
    {{TISHREI, 15, IGNORE_YEAR}, "Sukkot I", YOM_TOV_ENDS | IL_ONLY, NULL},
    {{TISHREI, 16, IGNORE_YEAR}, "Sukkot II", YOM_TOV_ENDS | CHUL_ONLY, NULL},
    {{TISHREI, 16, IGNORE_YEAR}, "Sukkot II (CH''M)", IL_ONLY, NULL},
    {{TISHREI, 17, IGNORE_YEAR}, "Sukkot III (CH''M)", 0, NULL},
    {{TISHREI, 18, IGNORE_YEAR}, "Sukkot IV (CH''M)", 0, NULL},
    {{TISHREI, 19, IGNORE_YEAR}, "Sukkot V (CH''M)", 0, NULL},
    {{TISHREI, 20, IGNORE_YEAR}, "Sukkot VI (CH''M)", 0, NULL},
    {{TISHREI, 21, IGNORE_YEAR}, "Sukkot VII (Hoshana Raba)", LIGHT_CANDLES, NULL},
    {{TISHREI, 22, IGNORE_YEAR}, "Shmini Atzeret", CHUL_ONLY | LIGHT_CANDLES_TZEIS, NULL},
    {{TISHREI, 22, IGNORE_YEAR}, "Shmini Atzeret", IL_ONLY | YOM_TOV_ENDS, NULL},
    {{TISHREI, 23, IGNORE_YEAR}, "Simchat Torah", CHUL_ONLY | YOM_TOV_ENDS, NULL},

    {{KISLEV, 24, IGNORE_YEAR}, "Chanukah: 1 Candle", CHANUKAH_CANDLES, NULL},
    {{KISLEV, 25, IGNORE_YEAR}, "Chanukah: 2 Candles", CHANUKAH_CANDLES, NULL},
    {{KISLEV, 26, IGNORE_YEAR}, "Chanukah: 3 Candles", CHANUKAH_CANDLES, NULL},
    {{KISLEV, 27, IGNORE_YEAR}, "Chanukah: 4 Candles", CHANUKAH_CANDLES, NULL},
    {{KISLEV, 28, IGNORE_YEAR}, "Chanukah: 5 Candles", CHANUKAH_CANDLES, NULL},
    {{KISLEV, 29, IGNORE_YEAR}, "Chanukah: 6 Candles", CHANUKAH_CANDLES, NULL},
    {{SHVAT, 15, IGNORE_YEAR}, "Tu BiShvat", 0, NULL},
    {{NISAN, 14, IGNORE_YEAR}, "Erev Pesach", LIGHT_CANDLES, NULL},
    {{NISAN, 15, IGNORE_YEAR}, "Pesach I", CHUL_ONLY | LIGHT_CANDLES_TZEIS, NULL},
    {{NISAN, 15, IGNORE_YEAR}, "Pesach I", IL_ONLY | YOM_TOV_ENDS, NULL},
    {{NISAN, 16, IGNORE_YEAR}, "Pesach II", CHUL_ONLY | YOM_TOV_ENDS, NULL},
    {{NISAN, 16, IGNORE_YEAR}, "Pesach II (CH''M)", IL_ONLY, NULL},
    {{NISAN, 17, IGNORE_YEAR}, "Pesach III (CH''M)", 0, NULL},
    {{NISAN, 18, IGNORE_YEAR}, "Pesach IV (CH''M)", 0, NULL},

    {{NISAN, 19, IGNORE_YEAR}, "Pesach V (CH''M)", 0, NULL},
    {{NISAN, 20, IGNORE_YEAR}, "Pesach VI (CH''M)", LIGHT_CANDLES, NULL},
    {{NISAN, 21, IGNORE_YEAR}, "Pesach VII", CHUL_ONLY | LIGHT_CANDLES_TZEIS, NULL},
    {{NISAN, 21, IGNORE_YEAR}, "Pesach VII", IL_ONLY | YOM_TOV_ENDS, NULL},
    {{NISAN, 22, IGNORE_YEAR}, "Pesach VIII", CHUL_ONLY | YOM_TOV_ENDS, NULL},

    {{IYYAR, 14, IGNORE_YEAR}, "Pesach Sheni", 0, NULL},

    {{IYYAR, 18, IGNORE_YEAR}, "Lag BaOmer", 0, NULL},

    {{SIVAN, 5, IGNORE_YEAR}, "Erev Shavuot", LIGHT_CANDLES, NULL},
    {{SIVAN, 6, IGNORE_YEAR}, "Shavuot I", CHUL_ONLY | LIGHT_CANDLES_TZEIS, NULL},
    {{SIVAN, 6, IGNORE_YEAR}, "Shavuot I", IL_ONLY | YOM_TOV_ENDS, NULL},
    {{SIVAN, 7, IGNORE_YEAR}, "Shavuot II", CHUL_ONLY | YOM_TOV_ENDS, NULL},
    {{AV, 15, IGNORE_YEAR}, "Tu B'Av", 0, NULL},
    {{ELUL, 1, IGNORE_YEAR}, "Rosh Hashana LaBehemot", 0, NULL},
    {{ELUL, 29, IGNORE_YEAR}, "Erev Rosh Hashana", LIGHT_CANDLES, NULL}};
...
holstorep_t getHolstorep(void) { /* return a pointer to a new holiday */
  holstorep_t tmp;
...
int PushHoliday(holstorep_t hp, /* the holiday to be added */
...
static void load_variable_holidays(int hYear) {
  ...
  PushHoliday(tmpholp, &var_holidays[TISHREI][roshHashana % 7L == THU ? 4 : 3]);
  ...
  if (hYear >= 5711 && !suppressModern_sw) { /* Yom HaShoah first observed in 1951 */
    long int nisan27;
    int nisan_day = 27;
  ...
  if (hYear > 5708 && !suppressModern_sw) { /* only really makes sense after 1948 */
    tmpholp = getHolstorep();
    ...
    if (passover % 7L == SUN)
      ...
      tempDt.dd = 5;
             ...
             TUE) /* no Yom Hazikaron on motzei shabbat allowed after 5764*/
      tempDt.dd = 6;
    else
      tempDt.dd = 5;
  ...
  if (hYear > 5727 && !suppressModern_sw) { /* only really makes sense after 1967 */
    tmpholp = getHolstorep();
  ...
  PushHoliday(tmpholp, &var_holidays[AV][tishaBav % 7L == SAT ? 9 : 8]);

  tmpholp = getHolstorep();
  tmpholp->name =
      tishaBav % 7L == SAT ? HOLIDAY_TISH_A_B_AV_OBSERVED : HOLIDAY_TISH_A_B_AV;
  PushHoliday(tmpholp, &var_holidays[AV][tishaBav % 7L == SAT ? 10 : 9]);
...
void init_user_holidays(int hyear) {
  ...
  for (; !feof(inFile); lineNum++, nextChar = (char)getc(inFile)) { /* force an EOF */
    ...
    if (isAllNums(monthStr)) {
      warn("Numeric hebrew month in input file.  Skipping line %s", hc_itoa(lineNum));
      continue;
    }
    ...
    if (inDay < 1 || inDay > 30) {
      warn("Date out of range in input file. Skipping line %s", hc_itoa(lineNum));
      continue;
    }
...
void init_yahrtzeits(int hyear) {
  ...
  for (; !feof(yFile); lineNum++, nextChar = (char)getc(yFile)) { /* force an EOF */
    ...
    if (!isAllNums(monthStr)) {
      warn("Non-numeric month in yahrtzeit file. Skipping line %s", hc_itoa(lineNum));
      continue;
    }
    sscanf(monthStr, "%d", &inMonth);

    if (inMonth > 12 || inMonth < 1 || inDay < 1 ||
        inDay > getMonthLength(inYear, inMonth)) {

      warn("Date out of range in yahrtzeit file. Skipping line %s", hc_itoa(lineNum));
      continue;
    }

    gDeath.tm_mday = inDay;
    gDeath.tm_mon = inMonth - 1;
    gDeath.tm_year = inYear - 1900;
    ...
    if (hDeath.mm == CHESHVAN && hDeath.dd == 30 && !long_cheshvan(hDeath.yy + 1)) {
      hDeath.dd = 1;
      hDeath.mm = KISLEV;
      hDeath.yy = hyear;
    ...
    else if (hDeath.mm == KISLEV && hDeath.dd == 30 && short_kislev(hDeath.yy + 1)) {
      hDeath.dd = 1;
      hDeath.mm = TEVET;
      hDeath.yy = hyear;
      hDeath = *abs2hebrew(hebrew2abs(&hDeath) - 1L);
    ...
    else if (hDeath.mm == ADAR_I && hDeath.dd == 30 && !LEAP_YR_HEB(hyear)) {
      hDeath.dd = 30;
      hDeath.mm = SHVAT;
    }
    ...
    if (hDeath.mm == CHESHVAN && hDeath.dd == 30 && !long_cheshvan(hyear)) {
      hDeath.mm = KISLEV;
      hDeath.dd = 1;
    } else if (hDeath.mm == KISLEV && hDeath.dd == 30 && short_kislev(hyear)) {
      hDeath.mm = TEVET;
      hDeath.dd = 1;
    }
...
/* It must therefore be called at the beginning of every hebrew year. */
{
  ...
  if (first) {
    for (m = 0, todayinp = inp_holidays; /* load constant holidays */
         m < LAST_INDEX(inp_holidays); m++, todayinp++) {
...
int getHebHolidays(const struct hebdate *dth, holstorep_t *holiList) {
  int tmpMask;
  ...
  *holiList = NULL;
  ...
  if (dth->dd == 1) {
     /* special processing for rosh hashana */
    if (dth->mm == TISHREI) {
      tmpholip = getHolstorep();
      initStr(&tmpholip->name, NM_LEN);
      sprintf(tmpholip->name, "%s %d", _("Rosh Hashana"), dth->yy);
