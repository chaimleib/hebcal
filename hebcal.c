...
int ashkenazis_sw, candleLighting_sw, gregDateOutputFormatCode_sw, hebrewDates_sw,
    inputFile_sw, israel_sw, latlong_sw, printOmer_sw, printMolad_sw,
    printSunriseSunset_sw, sedraAllWeek_sw, sedrot_sw, noGreg_sw, printHebDates_sw,
    printSomeHebDates_sw, noHolidays_sw, tabs_sw, weekday_sw, suppress_rosh_chodesh_sw,
    suppressModern_sw, sunsetAlways_sw, sunriseAlways_sw, default_zemanim, abbrev_sw,
    first_weekday, this_weekday, dafYomi_sw, utf8_hebrew_sw, yearDigits_sw,
    yahrtzeitFile_sw;
...
static struct _zman {
  ...
  int min_offset;
    ...
    {ZMAN_SZKS, "Kriat Shema, sof zeman", "Krias Shema, sof zeman",
     "Kriat Shema, sof zeman", 3, 0},
    {ZMAN_TEFILAH, "Tefilah, sof zeman", "Tefilah, sof zeman", "Tefilah, sof zeman", 4,
     0},
    ...
    {ZMAN_CANDLES_BEFORE, "Candle lighting", "Candle lighting",
     "\344\343\354\367\372 \360\370\345\372", 12, -18},
    ...
    {ZMAN_CANDLES_AFTER, "Candle lighting", "Candle lighting",
     "\344\343\354\367\372 \360\370\345\372", 12, 72}};
...
static int get_rise_set(const struct tm *todayGreg,
  ...
  *gmt_offset = timelib_get_current_offset(t) / 3600.0;
  rs = timelib_astro_rise_set_altitude(t, longitude, latitude, SUNRISE_SUNSET_ALTITUDE, 1,
...
void print_sunrise_sunset(const struct tm *todayGreg) {
  ...
  printf("%s:%2d:%02d; %s:%2d:%02d\n", "Sunrise", rise_hour, rise_minute, "Sunset",
         set_hour, set_minute);
}
...
void print_candlelighting_times(int mask, int weekday, const struct tm *todayGreg) {
  ...
  for (i_zman = 0; i_zman < num_zmanim; i_zman++) {
    ...
    if ((zemanim[i_zman].flags == ZMAN_MINCHA_GEDOLA) && var_hr_hours < 1) {
    ...
    if (zemanim[i_zman].flags == ZMAN_HAVDALAH) {
      printf("%s (%d min): %d:%02d\n", _(zman_name), havdalah_minutes, hour, minute);
    } else {
...
#define INCLUDE_TODAY(_sw) ((_sw) && ((!abbrev_sw) || (first_weekday == day_of_week)))
  ...
  while (todayAbs <= endAbs) {
    ...
    holidays_today = holip && (!noHolidays_sw || (returnedMask & USER_EVENT));
    molad_today = printMolad_sw && (day_of_week == SAT) &&
    ...
    if (candleLighting_sw) {
      if (day_of_week == FRI)
        ...
        if (returnedMask & LIGHT_CANDLES)
          today_zemanim |=
              (day_of_week == SAT) ? ZMAN_CANDLES_AFTER : ZMAN_CANDLES_BEFORE;
        else if ((returnedMask & LIGHT_CANDLES_TZEIS) && !(returnedMask & YOM_TOV_ENDS))
      ...
      if (!(today_zemanim & (ZMAN_CANDLES_BEFORE)) && (returnedMask & CHANUKAH_CANDLES))
    ...
    if (INCLUDE_TODAY(printHebDates_sw) ||
      ...
      if (utf8_hebrew_sw) {
        ...
        printf("%d%s of %s, %d\n", todayHeb.dd, numSuffix(todayHeb.dd),
               _(hMonths[LEAP_YR_HEB(todayHeb.yy)][todayHeb.mm].name), todayHeb.yy);
      }
    }
    ...
    if (molad_today) {
      ...
      printf("Molad %s: %s, %d minutes and %d chalakim after %d %s\n",
             ...
             (int)moladNext.chalakim / 18, moladNext.chalakim % 18,
