...
int max_days_in_heb_month(int month, int year) {
  if (month == IYYAR || month == TAMUZ || month == ELUL || month == TEVET ||
      month == ADAR_II || (month == ADAR_I && !LEAP_YR_HEB(year)) ||
...
molad_t get_molad(int year, int month) {
  ...
  m_adj -= 7;
  if (m_adj < 0)
    m_adj += MONTHS_IN_HEB(year);

  yearl = (long)year;
  m_elapsed = m_adj + 235L * ((yearl - 1L) / 19L) + 12L * ((yearl - 1L) % 19) +
              ((((yearl - 1L) % 19L) * 7) + 1L) / 19L;

  p_elapsed = 204L + (793L * (m_elapsed % 1080));

  h_elapsed =
      5L + (12L * m_elapsed) + 793L * (m_elapsed / 1080L) + p_elapsed / 1080L - 6L;
...
static long int hebrew_elapsed_days(int year) {
  ...
  yearl = (long)year;
  m_elapsed = 235L * ((yearl - 1L) / 19L) + 12L * ((yearl - 1L) % 19L) +
  ...
  if ((alt_day % 7L) == 0L || (alt_day % 7L) == 3L || (alt_day % 7L) == 5L)
    return alt_day + 1L;
  else
    return alt_day;
}
...
int long_cheshvan(int year) { return ((days_in_heb_year(year) % 10) == 5); }

/* true if Kislev is short in hebrew YEAR */
int short_kislev(int year) { return ((days_in_heb_year(year) % 10) == 3); }
